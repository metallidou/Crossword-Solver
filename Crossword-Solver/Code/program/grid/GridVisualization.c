...
{
    GridVisual grid = malloc(sizeof(struct GridVISUALIZATION));
    int x, y, i, j;
    ...
    grid->IsBlackBox = malloc(x * sizeof(bool*));
    for(i = 0; i < x; i++)
        grid->IsBlackBox[i] = malloc(y * sizeof(bool));

    // at first we assume that all boxes are white
    for(i = 0; i < x; i++)              
    {
        for(j = 0; j < y; j++)
            grid->IsBlackBox[i][j] = false;
    }    
